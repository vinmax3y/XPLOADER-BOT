...
lolcatjs.fromString("[Successfully connected to the Local database]");
}
...
async function downloadSessionData() {
  try {
   ...
   const sessdata = global.SESSION_ID.split("XPLOADER~")[1];
      ...
      const decodedBuffer = Buffer.from(sessdata, 'base64');

      // Write the buffer to the creds.json file
      await fs.promises.writeFile(credsPath, decodedBuffer);
      console.log(color(`Xploader session successfully saved, please wait!!`, 'green'));

      // Start the Xploader bot
      await startXploader();
    }
  } catch (error) {
...
let nomer = await Xploader.sendTextWithMentions(kopel.from, `My owner cannot receive ${kopel.isVideo ? `video` : `audio`} calls at the moment.\n\nSorry @${kopel.from.split('@')[0]} Xploader Bot is now blocking you for causing disturbance.\n\nIf you called by mistake please look for means to contact my owner to be unblocked!`)
await sleep(8000)
...
app.listen(port, (err) => {
    if (err) {
        ...
        console.log(color(`Xploader is running on port: ${port}`, 'yellow'));
    }
});
